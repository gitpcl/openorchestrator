...
- **Multi-provider plugin layer** — built-in support for **Claude Code, Pi, OpenCode, and Droid**; register custom AI tools via config without code changes. `owt new --ai-tool <name>` picks the engine per worktree; auto-detection picks the best installed tool when you don't.
