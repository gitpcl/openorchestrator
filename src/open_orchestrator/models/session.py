...
class SessionData(BaseModel):
    ...
    session_id: str | None = Field(
        default=None,
        description="Claude session ID if known"
    )
    session_dir: str | None = Field(
        default=None,
        description="Path to the .claude directory containing session data"
    )
    copied_from: str | None = Field(
        default=None,
        description="Source worktree name if this session was copied"
    )
    copied_at: datetime | None = Field(
        default=None,
        description="When the session was copied"
    )
    original_session_id: str | None = Field(
        default=None,
        description="Original session ID from source worktree"
    )
    data_paths: list[str] = Field(
        default_factory=list,
        description="Paths of copied session data files"
    )
...
class SessionCopyResult(BaseModel):
    ...
    status: SessionCopyStatus = Field(
        ...,
        description="Status of the copy operation"
    )
    source_worktree: str = Field(
        ...,
        description="Source worktree name"
    )
    target_worktree: str = Field(
        ...,
        description="Target worktree name"
    )
    files_copied: list[str] = Field(
        default_factory=list,
    ...
    session_id: str | None = Field(
        default=None,
        description="Session ID of the copied session"
    )
    copied_at: datetime = Field(
        default_factory=datetime.now,
        description="When the copy was performed"
    )
...
class SessionStore(BaseModel):
    ...
    sessions: dict[str, SessionData] = Field(
        default_factory=dict,
        description="Map of worktree name to session data"
    )
    ...
    def get_sessions_copied_from(self, source_worktree: str) -> list[SessionData]:
        ...
        return [
            ...
            if s.copied_from == source_worktree
        ]
